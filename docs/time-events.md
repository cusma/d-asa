...
conventions* (ID<`255`, see Day-Count Conventions section), the *time periods* between
subsequent events **SHOULD** be multiples of a day, in seconds (`86400`).
...
> 📎 **EXAMPLE**
>
> The following are invalid time events (UNIX times):
>
> ```text
...
> 📎 **EXAMPLE**
>
> A D-ASA with `K` coupons, has the following time events:
>
> ```text
...
> each paid on December 31st 00:00:00 GMT+0, has the following time events (UNIX
> times):
>
> ```text
...
> the following time events (UNIX times):
>
> ```text
...
> A D-ASA whose primary distribution lasts from December 1st, 2023 00:00:00 GMT+0
> to December 15th, 2023 00:00:00 GMT+0, is issued on January 1st, 2024 00:00:00
> GMT+0 and no maturity, with perpetual coupons, has the following time events (UNIX
> times):
>
> ```text
